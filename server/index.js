...
////magazine fea
//rating
app.post("/mgsubmit-rating", async (req, res) => {
...
app.post("/mgsubmit-comment", async (req, res) => {
...
app.get("/mgcomments/:magName", async (req, res) => {
  const { magName } = req.params;

  try {
    const comments = await Comment.find({ magName }).sort({ createdAt: -1 });
    res.json(comments);
  } catch (error) {
    console.error(error);
    res
...
//get comment
app.post("/mgget-comment", async (req, res) => {
  const { comments } = await req.body;
  console.log(comments);
  try {
    // if (books == "books") {
...
app.post("/mgadd-to-favorites", async (req, res) => {
  ...
  try {
    ...
    // Check if the book is already in favorites
    if (!user.favorites.includes(magazineId)) {
      user.favorites.push(magazineId);
      await user.save();
      res.json({ message: "Book added to favorites" });
    } else {
      res.status(400).json({ message: "Book is already in favorites" });
...
// Endpoint to remove a book from favorites
app.post("/mgremove-from-favorites", async (req, res) => {
  const { userId, magazineId } = req.body;
  ...
  try {
    ...
    // Check if the book is in favorites before attempting to remove it
    if (user.favorites.includes(magazineId)) {
      user.favorites.pull(magazineId); // mongoose method to remove item from array
      await user.save();
      res.json({ message: "Book removed from favorites" });
    } else {
      res.status(400).json({ message: "Book is not in favorites" });
...
app.post("/mgis-favorited", async (req, res) => {
  const { userId, magazineId } = req.body;
  ...
  try {
    ...
    const isFavorited = user.favorites.includes(magazineId);
    res.json({ isFavorited });
  } catch (error) {
    console.error(error);
    res
